...
export const LayerTemplateManager: React.FC<LayerTemplateManagerProps> = ({
  ...
  const handleCreateCategory = async () => {
    ...
    const newCategory: CategoryDefinition = {
      ...
      defaultLayers: JSON.parse(JSON.stringify(PRESET_CATEGORIES[0].defaultLayers)),
      isCustom: true
    };
    ...
    const defaultTmpl: CustomTemplate = {
      ...
      categoryId: catId,
      layers: JSON.parse(JSON.stringify(newCategory.defaultLayers)),
      createdAt: Date.now(),
      updatedAt: Date.now(),
      isPreset: false
    };
    ...
    setNewCatName('');
    showFeedback(`"${newCategory.name}" kategorisi başarıyla oluşturuldu!`);
  };

  // Özel Kategori Sil
  const handleDeleteCategory = async (catId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (confirm('Bu özel kategoriyi silmek istediğinize emin misiniz?')) {
      await db.categories.delete(catId);
      // Bu kategoriye ait şablonları da sil
  ...
  return (
    <div className="glass-panel rounded-2xl p-4 border border-slate-800 shadow-xl space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-semibold text-white">Kategori Seçimi</h3>
              <p className="text-xs text-slate-400">Çalışmak istediğiniz kategoriyi seçin veya yeni kategori oluşturun</p>
            </div>
          </div>
        ...
        {isCreatingCategory && (
          <div className="p-3.5 rounded-xl bg-slate-950 border border-red-500/50 space-y-3 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div>
                <label className="text-[10px] text-slate-400 font-medium">Kategori Adı</label>
                <input
                  type="text"
                  value={newCatName}
                  onChange={(e) => setNewCatName(e.target.value)}
                  placeholder="Örn: Spor, Magazin..."
              ...
              <div>
                <label className="text-[10px] text-slate-400 font-medium">Vurgu Rengi</label>
      ...
      {/* 2. KATLANIR KART: ŞABLON SEÇİMİ VE KAYDETME */}
      <div className="rounded-xl bg-slate-900/70 border border-slate-800 overflow-hidden transition-all shadow-md">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-200">Şablon Seçimi & Kaydetme</span>
            <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-800 text-slate-300 font-medium">
              {currentTemplate?.name || 'Seçili Şablon'}
            </span>
          </div>
      ...
      <div className="rounded-xl bg-slate-900/70 border border-slate-800 overflow-hidden transition-all shadow-md">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-200">Görsel Katmanları & Tikler</span>
            <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-800 text-emerald-400 font-medium">
              {activeLayersCount} / {slide.layers.length} Aktif
            </span>
          </div>
          <div className="text-slate-400">
          ...
          <div className="p-4 border-t border-slate-800/80 space-y-3 bg-slate-950/40 animate-fadeIn">
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-slate-400">
                Görselleri tiklerle açıp kapatabilir, konumlarını ayarlayabilirsiniz
              </span>
              <label className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] font-medium cursor-pointer transition-colors">
